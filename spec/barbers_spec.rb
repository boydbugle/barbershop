   ...
   describe("#name") do
    ...
    describe("#clients") do
      it("returns an array of clients for that barber") do
        ...
        test_barber.save()
        test_client = Client.new({:name => "Learn SQL", :barbers_id => test_barber.id(), :cut_description => "box cut with  comb and scissors", :booking_date => "02/08/2017"})
        test_client.save()
        test_client2 = Client.new({:name => "Review Ruby", :barbers_id => test_barber.id(), :cut_description => "beard shave with smoother", :booking_date => "02/09/2017"})
        test_client2.save()
    ...
    it("deletes a barber's clients from the database") do
        barber = Barber.new({:name => "Punk morio", :id => nil})
        barber.save()
        client = Client.new({:name => "learn SQL", :barbers_id => barber.id(), :cut_description => "box cut with  comb and scissors", :booking_date => "02/08/2017"})
        client.save()
        client2 = Client.new({:name => "Review Ruby", :barbers_id => barber.id(), :cut_description => "beard shave with smoother", :booking_date => "02/09/2017"})
        client2.save()
        barber.delete()
